...
class TagComparisonResult(Base):
    ...
    max_similarity = Column(Float(precision=3), nullable=False, comment="最大匹配相似度")
    optimal_matching = Column(Float(precision=3), nullable=False, comment="最优匹配相似度")
    threshold_matching = Column(Float(precision=3), nullable=False, comment="阈值匹配相似度")
    average_similarity = Column(Float(precision=3), nullable=False, comment="平均相似度")
